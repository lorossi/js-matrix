...
class Chain {
  ...
  _reset() {
    ...
    this._duration_factor = Math.floor(
    ...
    this._flash = [...new Array(flash_length)].map(() =>
      Math.round(Math.random() * this._duration)
    );
  ...
  _generateStatuses() {
    ...
    for (let i = 0; i < this._total_statuses; i++) {
      ...
      for (let j = 0; j < this._length; j++) {
        ...
        this._statuses[i][j] = String.fromCodePoint(
          0x3041 + Math.floor(Math.random() * 84)
        );
      }
    }
  }
  ...
  move(frame_count) {
    ...
    this._current_y =
      this._start_y +
      (this._canvas_size + this._scl * this._length) *
        percent *
        this._duration_factor;
    while (this._current_y > this._canvas_size)
      this._current_y -= this._canvas_size + this._scl * this._length;
    // select current status
    this._status_counter =
      Math.floor(percent * this._total_statuses + this._offset) %
      this._total_statuses;
